Ext.define('Admin.Tab.MintTreatments', {
    extend: 'Ext.panel.Panel',
    name: 'Admin.Tab.MintTreatments',
    id: 'tabMintTreatments',
    title: 'MintTreatments',
    closable: true,
    layout: 'fit',
    bodyBorder: false,
    ...
    __src_grid : function(){
        ...
        var store_mds = Ext.create('Ext.data.Store', {
            ...
            fields: [
                ...
                type: 'ajax', url: App.url('minttreatments'), extraParams: { action: 'listmd' },
        ...
        this.grid = Ext.create('Ux.Grid', {
            ...
            tbar : [
                ...
                '->',
                { xtype: 'button', text: 'Approve / Reject multiple treatments', handler: this.on_approve_multiple_minttreatments, scope: this }
            ],
            pageSize: 300, autoload: true, border: false, url : App.url('minttreatments'), params : { action: 'grid' }, model:'Modulo', remoteSort: true,
            columns: [
               ...
               `<p><b>Mint Treatments:</b> {minttreatments}</p>
                    <p><b>Injector\'s Notes :</b> {notes}</p>
                    <p><b>Doctor\'s Notes :</b> {doctor_notes}</p>
                    <p><b>Associated certificates:</b> 
                        <tpl for="certificates">
                            <a class="cert-link" target="blank" href="{[this.get_url(values)]}">{name}</a>
                        </tpl>
                    </p>
                    <p><b>Associated GFE documents:</b> 
                        <tpl for="certificates">
                            <a class="gfe-link" target="blank" href="{[this.get_gfe_url(values)]}">{name}</a>
                        </tpl>
                    </p>
                    <p><tpl for="files">
        ...
        Ext.create('Wnd.ApprMintTreatment', {
            app: {record: record, callback: Ext.bind(function(){ this.reload(); }, this)}
        }).show();
    }, on_approve_multiple_minttreatments: function() {
...
Ext.define('Wnd.ApprMintTreatment', {
    extend: 'Ext.window.Window',
    title: 'Evaluate mint treatment', modal: true, border: false, width: 600, layout: 'fit', //closeAction: 'hide',
    minimizable: false,
